...
from transformers.pytorch_transformers.modeling_bert import (
    BertEmbeddings, BertSelfAttention, BertAttention, BertEncoder, BertLayer,
    BertSelfOutput, BertIntermediate, BertOutput, BertPooler, BertLayerNorm,
    BertPreTrainedModel, BertPredictionHeadTransform, BertOnlyMLMHead,
...
class ImageBertForMultipleChoice(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        reshaped_pool_output = pooled_output.view(-1, self.config.num_choice *
...
class BertForImageCaptioning(CaptionPreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, curr_ids, past=None):
        ...
        if past is None:
            ...
            def _remove_rows_cols(t, row_start, row_end, col_start, col_end):
                ...
                res = torch.cat([
                    torch.cat([t00, t01], dim=2), torch.cat([t10, t11], dim=2)
                ],
                                dim=1)
            ...
            if self.prev_encoded_layers is None:
                ...
                self.full_attention_mask = torch.cat([
                    torch.cat([i2i, i2s], dim=2), torch.cat([s2i, s2s], dim=2)
                ],
                                                     dim=1)
            else:
                assert start_pos > 1
    ...
    def generate(
        ...
        self.full_attention_mask = self._expand_for_beams(attention_mask,
                                                          num_expand)
        self.full_masked_pos = self._expand_for_beams(masked_pos, num_expand)
        self.full_token_type_ids = self._expand_for_beams(token_type_ids,
                                                          num_expand)
        self.full_position_ids = self._expand_for_beams(position_ids,
                                                        num_expand)
