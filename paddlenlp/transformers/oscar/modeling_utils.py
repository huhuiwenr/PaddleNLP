...
from transformers.pytorch_transformers.modeling_bert import (
    BertConfig, load_tf_weights_in_bert, BERT_PRETRAINED_MODEL_ARCHIVE_MAP,
    BertPreTrainedModel)
from transformers.pytorch_transformers.modeling_utils import (
    PreTrainedModel, WEIGHTS_NAME, TF_WEIGHTS_NAME)
...
class CaptionPreTrainedModel(BertPreTrainedModel):
        ...
        while cur_len < max_length:
            ...
            if do_sample:
                ...
                next_token = torch.multinomial(
                    F.softmax(
                        next_token_logits, dim=-1), num_samples=1).squeeze(1)
            else:
                # Greedy decoding
    ...
    def _generate_beam_search(
        ...
        generated_hyps = [
            BeamHypotheses(
                num_keep_best, max_length, length_penalty, early_stopping=False)
        ...
        while cur_len < max_length:
            ...
            if do_sample:
                ...
                next_words = torch.multinomial(
                    F.softmax(
                        scores, dim=-1), num_samples=TOPN_PER_BEAM
            ...
            for batch_ex in range(batch_size):
                ...
                for idx, score in zip(next_words[batch_ex],
                    ...
                    if word_id.item(
                        ...
                        generated_hyps[batch_ex].add(input_ids[
                            batch_ex * num_beams + beam_id, :cur_len].clone(),
                                                     score.item())
                    else:
        ...
        for i, hypotheses in enumerate(generated_hyps):
            ...
            for best_idx, hyp_idx in enumerate(best_indices):
                ...
                tgt_len[i, best_idx] = len(
                    best_hyp) + 1  # +1 for the <EOS> symbol
        ...
        for batch_idx, best in enumerate(all_best):
            for best_idx, hypo in enumerate(best):
                decoded[batch_idx, best_idx, :tgt_len[batch_idx, best_idx] -
                        1] = hypo
                decoded[batch_idx, best_idx, tgt_len[batch_idx, best_idx] -
                        1] = eos_token_ids[0]
            ...
            if pretrained_model_name_or_path in cls.pretrained_model_archive_map:
                ...
                logger.error(
                    ...
                    format(pretrained_model_name_or_path, ', '.join(
                        cls.pretrained_model_archive_map.keys()), archive_file))
            return None
